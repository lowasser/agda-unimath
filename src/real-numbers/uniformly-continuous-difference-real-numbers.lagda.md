# Subtraction on the real numbers is uniformly continuous

```agda
{-# OPTIONS --lossy-unification #-}

module real-numbers.uniformly-continuous-difference-real-numbers where
```
...
open import real-numbers.uniformly-continuous-addition-real-numbers
...
[Subtraction on the real numbers](real-numbers.difference-real-numbers.md) is a
...
```agda
abstract
  is-uniformly-continuous-diff-ℝ :
    {l1 l2 : Level} →
    is-uniformly-continuous-binary-map-Metric-Space
      ...
      ( diff-ℝ)
  is-uniformly-continuous-diff-ℝ {l1} {l2} =
    right-comp-uniformly-continuous-binary-map-Metric-Space
      ...
      ( id)
      ( is-uniformly-continuous-map-id-Metric-Space (metric-space-leq-ℝ l1))
      ( neg-ℝ)
